  ...
  const handleEdit = async () => {
    ...
    if (croppedImage && image) {
      ...
      try {
        ...
        if (!signedUrl)
          return toast.error(
            "An error occurred while uploading the thumbnail.Please update again."
          );
        const blob = await fetch(croppedImage).then(
      ...
      <Transition.Root show={open} as={Fragment}>
        ...
        >
          <Transition.Child
            ...
            leaveTo="opacity-0"
          >
            <div className="fixed inset-0 my-10 bg-gray-500 bg-opacity-75 transition-opacity" />
          </Transition.Child>

          <div className="z-100 no-scrollbar fixed inset-0 flex items-center justify-center overflow-y-auto p-4 py-10 pt-20 sm:p-6">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
                      ...
                      <div className="mt-3 w-full text-center sm:text-left">
                              ...
                              {croppedImage ? (
                                <>
                                  <img
                                    ...
                                    alt="Cropped"
                                    className="w-full max-w-xs cursor-pointer rounded-md"
                                    onClick={() =>
                                      document
                                ...
                                <>
                                  <div className="mt-4 flex py-1 text-sm leading-6 text-gray-400">
                                    <Label
                                      htmlFor="file-upload"
                                      className="relative cursor-pointer rounded-md bg-[#181818] p-1 font-semibold text-purple-800 focus-within:outline-none focus-within:ring-2 focus-within:ring-purple-800 focus-within:ring-offset-2 hover:text-purple-700"
                                    >
                        ...
                        <div className="mt-4 flex flex-col gap-2">
                          ...
                          <Input
                            ...
                            value={user.title}
                            className="w-full rounded-md border-0 bg-zinc-800 py-1.5 text-gray-200  shadow-sm placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-gray-600 sm:text-sm"
                          />
                          <Label
                            htmlFor="description"
                            className="mt-4 block text-sm font-medium leading-6 text-gray-200"
                          >
                            Description
                          </Label>
                          <Textarea
                            ...
                            onChange={handleInputChange}
                            className="w-full rounded-md border-0 bg-zinc-800 py-1.5 text-gray-200   shadow-sm placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-purple-600 sm:text-sm"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="mt-5 flex flex-col gap-2 sm:mt-4 sm:flex-row sm:justify-end">
                      <Button
                        onClick={() => handleEdit()}
                        variant="default"
                        className="w-full bg-neutral-800 text-neutral-200 hover:bg-neutral-700 sm:w-auto"
                        disabled={loading}
                      >
                        {loading ? (
                          <>
                      ...
                      <Button
                        ...
                        className="w-full bg-gray-700 text-gray-200 hover:bg-gray-600 sm:w-auto"
                      >
                        Cancel
                      </Button>
                    </div>
