...
export const userRouter = createTRPCRouter({
  ...
  updateUser: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.handle) {
        const existingHandle = await ctx.prisma.user.findFirst({
          where: {
            handle: input.handle,
          },
        });
        if (existingHandle && existingHandle.id !== input.id) {
          throw new Error("Handle already exists. Please choose another one");
        }
      }
