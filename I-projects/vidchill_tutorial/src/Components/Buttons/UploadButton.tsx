...
export function UploadButton({ link, refetch }: { link?: string, refetch: () => Promise<unknown> }) {
  ...
  const handlevideo = async () => {
    ...
    if (uploadedVideo.type !== "video/mp4") return toast.error("Please attach a video with mp4 format")
    setLoading(true)
    ...
    try {
      ...
      if (!signedUrl) return toast.error("An error occurred while uploading the video")
      ...
      addVideoUpdateMutation.mutate(newVideoData, {
        onSuccess: () => {
          ...
          setLoading(false)
          void refetch()
          link ? toast.info("Video uploaded successfully.Please publish it.", {
            duration: 4000,
            action: {
  ...
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0 && e.target.files[0]?.type === "video/mp4") {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg bg-[#181818] text-gray-200 border border-gray-700">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold leading-6 text-gray-200">Upload Video</DialogTitle>
        </DialogHeader>
