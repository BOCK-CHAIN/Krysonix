...
export default function FollowButton({ followingId, hideIcon, viewer, setLoading, refetch, loading }: FollowButton) {
  const { data: sessionData } = useSession()
  const [userChoice, setUserChoice] = useState({
    following: viewer.hasFollowed,
  })
  const addFollowMutation = api.user.addFollow.useMutation()
  const handleFollow = async (input: { followingId: string; followerId: string }) => {
    if (setLoading) {
      setLoading(true)
    }
  ...
  return (
    <>
      <Button
        ...
        className={`flex ${userChoice.following
          ? "bg-gray-700 text-gray-200 hover:bg-gray-600"
          : "bg-purple-800 text-gray-200 hover:bg-purple-800"
          }`}
      >
        {loading ? <><Loader state></Loader></> :
          <>
            <UserPlus
